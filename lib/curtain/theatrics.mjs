...
// Age ramp: faint → fainter → gone. Motes stay background atmosphere, not
// foreground sparkle (was "· ˙ ʼ" which read too loud on stage-black cards).
const DRIFT_GLYPHS_DEFAULT = ["·", "˙", " ", " "];
...
export const driftField = (
  ...
  rows,
  t,
  { seed = 0, dir = "lateral", density = 0.04, glyphs, fade = true } = {},
) => {
  const c = Math.max(0, Math.floor(Number(cols) || 0));
  const r = Math.max(0, Math.floor(Number(rows) || 0));
  ...
  const dens = Number.isFinite(Number(density)) ? Number(density) : 0.04;
 ...
 */
// Soft cells denizen may replace: empty or particle glyphs (motes/burst).
// Theme base art (anvil, labels) is never soft → art stays sacred.
const isSoftCell = (ch) => !ch || ch === " " || "·˙ʼ'*+.".includes(ch);
...
export const mergeDenizen = (baseLines, cel, zone = {}) => {
  ...
  for (let i = 0; i < cel.length; i++) {
    ...
    for (let j = 0; j < art.length; j++) {
      ...
      // Base theme ink wins; denizen may cover empty space and motes.
      if (isSoftCell(chars[col] ?? " ")) chars[col] = a;
    }
    out[r] = chars.join("");
  }
  return out;
...
/**
 * Apply Act I/II theatrics onto already-rendered plain card lines.
 * Does not mutate theme art data — only composites over the painted grid.
 * Order: base → motes/burst → denizen → curtain fabric LAST.
 *
 ...
 *   sparkFrames?: number,
 *   denizen?: string[]|null,
 *   denizenZone?: {top?: number, left?: number}|null,
 * }} opts
...
export const applyTheatrics = (plainLines, opts = {}) => {
  ...
  // Keep density low so creatures + labels stay the foreground.
  if (effects.motes) {
  ...
  if (effects.burst && tick < sparkFrames) {
    ...
    const density = Math.max(0, 0.1 * (1 - life)); // softer than before
  ...
  // Order is load-bearing: motes above; curtain fabric overlay below paints LAST.
