...
export const applyTheatrics = (plainLines, opts = {}) => {
  ...
  if (effects.sparkRain && tick < sparkFrames) {
    const t =
      sparkFrames <= 1 ? 1 : Math.min(1, tick / Math.max(1, sparkFrames - 1));
    const sparks = sparkRain(cols, rows, t, { palette: "done" });
    lines = mergeSparks(lines, sparks);
  }
