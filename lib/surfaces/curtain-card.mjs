...
export const renderCard = (
  ...
  // Corner chrome (× off / ↻ pet). Classic: keep body SGR, rewrite bottom row.
  // Composite: paint chrome on plain grid, color whole card, dim chrome strip.
  if (!wantComposite) {
    return paintChromeOnColoredCard(out, cols, rows, solid, bgSpec);
  }

  // Composite stage curtain / motes over plain geometry (creature is already
  // the main art block — not layered again). Recolor after.
  let composed = applyTheatrics(out, {
    cols,
    rows,
    effects,
  ...
  composed = paintChromePlain(composed, cols, rows);

  const colored = composed.map((plainLine) => {
    if (solid) {
      return color(
  ...
  return styleChromeDim(colored, cols, rows);
};

/** Overlay dim chrome on an already-colored card without recoloring the body. */
const paintChromeOnColoredCard = (coloredLines, cols, rows, solid, bgSpec) => {
  const layout = layoutChrome(cols, rows);
  if (!layout.length) return coloredLines;
  ...
  for (const b of layout) {
    for (let i = 0; i < b.label.length; i++) {
      const col = b.c0 + i;
      if (col >= 0 && col < cols && (chars[col] === " " || !chars[col])) {
        chars[col] = b.label[i];
      }
    }
  }
  // Full-width solid bg (matches blankLine), chrome glyphs in gray.
  let line = "";
  for (let col = 0; col < cols; col++) {
    ...
    if (solid) {
      line +=
        ch === " "
          ? color(" ", { bg: bgSpec })
          : color(ch, { bg: bgSpec, fg: onBtn ? "gray" : "white" });
    } else if (ch === " ") {
      line += " ";
    } else {
