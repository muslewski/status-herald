...
import { nextSpecies, seedFor, speciesFor } from "./denizens.mjs";
import {
  isBgTaskStart,
  isLoopPrompt,
  isMonitorStart,
...
import { notifyOnNeedsEdge } from "./sound.mjs";
...
// Absolute hook body for session window-unlinked (installed on arm).
// tmux 3.x has no after-kill-window; window-unlinked fires when prefix+& etc.
// unlinks a window. If that was the last live window, only _curtain remains
// and the grey card would stick forever without this prune.
const HERALD_BIN = fileURLToPath(new URL("../../bin/herald", import.meta.url));
export const afterKillWindowHookCmd = () => {
  // Quote node + bin so paths with spaces survive tmux run-shell.
  const node = process.execPath.replace(/'/g, `'\\''`);
  const bin = HERALD_BIN.replace(/'/g, `'\\''`);
  return `run-shell '${node} ${bin} curtain after-kill-window "#{session_name}"'`;
};
...
export const arm = (sess, t = realTmux, cfg = loadConfig().curtain) => {
  if (t.getSessOpt(sess, "@herald_armed") === "1") {
    // Already armed: still refresh kill-prune hook (upgrade path for live fleets).
    t.setDetachOnDestroy?.(sess);
    t.setHook?.(sess, "window-unlinked", afterKillWindowHookCmd());
    return;
  }
  ...
  // Client leaves cleanly when this session is destroyed (no grey frozen pane).
  t.setDetachOnDestroy?.(sess);
  // prefix+& / kill-window → window-unlinked (tmux 3.x; not after-kill-window).
  // If live work dies and only _curtain remains, prune it.
  t.setHook?.(sess, "window-unlinked", afterKillWindowHookCmd());
};

/**
 * Called from tmux window-unlinked on an armed session.
 * If the operator killed the last live window (prefix+& etc.), only `_curtain`
 * remains and the grey card would stick forever — kill the card so the session
 * (and the curtain) die with the work.
 */
export const afterWindowKill = (sess, t = realTmux, cfg = loadConfig().curtain) => {
  if (!sess) return;
  if (t.getSessOpt(sess, "@herald_armed") !== "1") return;

  const wins =
    typeof t.listWindowsInSession === "function"
      ? t.listWindowsInSession(sess)
      : [];
  // Session already destroyed — nothing left to clean.
  if (!wins || wins.length === 0) return;

  const nonCard = wins.filter((w) => w.name !== CARD_WIN);
  const card = wins.find((w) => w.name === CARD_WIN);

  // Only the grey card remains → kill it (last window ends the session).
  if (nonCard.length === 0 && card) {
    t.killWindow(`${sess}:${CARD_WIN}`);
    return;
  }

  // Live window id is stale (killed) but other live windows remain: retarget.
  const live = t.getSessOpt(sess, "@herald_live_win");
  if (live && !wins.some((w) => w.id === live) && nonCard[0]) {
    t.setSessOpt(sess, "@herald_live_win", nonCard[0].id);
    if (t.getSessOpt(sess, "@herald_covered") !== "1") {
      t.selectWindow(nonCard[0].id);
    }
  }

  // Card was killed but session still has live work: clear covered, stay usable.
  if (!card && nonCard.length > 0) {
    t.setSessOpt(sess, "@herald_covered", "0");
...
/**
 * Cycle curtain denizen (fox → cat → owl → …). Fun chrome control from the card.
 * Writes @herald_entity (+ bumps seed so the pose phase shifts).
 */
export const cycleDenizen = (sess, t = realTmux, cfg = loadConfig().curtain) => {
  if (!sess) return "";
  if (t.getSessOpt(sess, "@herald_armed") !== "1") return "";
  const cur =
    t.getSessOpt(sess, "@herald_entity") || speciesFor(sess, cfg) || "";
  const next = nextSpecies(cur);
  if (!next) return cur;
  t.setSessOpt(sess, "@herald_entity", next);
  const seed = (Number(t.getSessOpt(sess, "@herald_seed")) || seedFor(sess)) + 1;
  t.setSessOpt(sess, "@herald_seed", seed);
  return next;
};
...
export const stampFromHook = (
  ...
  // Attention sound: edge into NEEDS only (fail-open; never blocks hooks).
  try {
    const covered = t.getSessOpt(sess, "@herald_covered") === "1";
    const lastFireSec = Number(t.getSessOpt(sess, "@herald_sound_last")) || 0;
    notifyOnNeedsEdge({
      soundCfg: cfg?.sound,
      prevState: cur,
      nextState: next,
      ...
      nowSec,
      lastFireSec,
      setLastFire: (sec) => t.setSessOpt(sess, "@herald_sound_last", sec),
      spawn: deps.spawn,
    });
  } catch {
    /* sound never owns curtain truth */
  }
};
