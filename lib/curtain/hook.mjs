...
export const parseHookPayload = (raw) => {
  ...
  return {
    ...
    loopPrompt:
      ...
      !synthetic &&
      /^\/loop\b/i.test(String(p.prompt || "").trim()),
...
// Grok /loop, monitor, and bg shells outlive a single Stop. Claude shells in
// background_tasks already appear as shells; these are synthesis-only signals.
export const isWatchStart = (ev) => {
  if (!ev) return false;
  if (ev.loopPrompt) return true;
  const n = String(ev.toolName || "")
    .toLowerCase()
    .replace(/[^a-z_]/g, "");
  if (n === "scheduler_create" || n === "monitor") return true;
  if (
    (n === "run_terminal_command" || n === "bash" || n === "bashoutput") &&
    ev.toolBackground
  )
    return true;
  return false;
...
export const nextState = (cur, ev, stored = {}) => {
  ...
  switch (ev.event) {
    case "UserPromptSubmit":
      // Automated task-complete injects look like prompts but must not pull a
      // finished card back to WORKING (Grok task-completed-* promptIds).
      if (ev.synthetic) return cur;
