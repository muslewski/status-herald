...
export const DEFAULTS = {
  curtain: {
    ...
    themeBySession: {},
    themes: {},
    // Motion theatrics (Act I). enabled/reducedMotion gate draw + spark rain +
    // NEEDS breathe. classic theme stays static regardless. drawFrames @ ~drawMs
    // total rides the existing card tick loop (no separate event loop).
    animation: {
      enabled: true,
      fps: 2,
      reducedMotion: false,
    ...
    // 300s quiet survives deep thinking (no hook events mid-reason).
    settle: {
    ...
    // Lease TTLs under curtain.lease (see lib/curtain/lease.mjs).
    lease: {
      subagentTtlSec: 300,
      watcherTtlSec: 900,
      bgShellTtlSec: 300,
    ...
    // Optional info lines on the curtain card (default off).
    lines: { model: false, sageZone: false },
    // Attention sound (default silent). Fires on edge into NEEDS only.
    // Backends are empty until the user opts in — no Mac/host assumptions.
    sound: {
      enabled: false,
      mode: "day", // day | night | off
      events: ["needs"],
  ...
  bars: {
    ...
    segments: {
      ...
      sage: { enabled: false, priority: 25 },
    },
  },
};

/** Resolved path to the user config JSON (HERALD_CONFIG or XDG). */
