...
// carry background_tasks and are authoritative. Notification carries none -- its
// only keys are `message` and `notification_type` -- so it has to fall back on
// what the last task-bearing event told us, which the session stores.
...
export const nextState = (cur, ev, stored = {}) => {
  ...
  switch (ev.event) {
    ...
    case "Notification":
      ...
      // idle_prompt is the only end-marker it will ever send. But "the main
      // agent is idle" is also precisely what a main agent looks like while it
      // waits on its subagents -- observed live, an idle_prompt landed 60s after
      // a Stop that carried three running subagents. It must not call that done.
      // It also fires behind an unanswered permission prompt, which still wins.
