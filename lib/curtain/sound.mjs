...
export const shouldFireSound = (cfg, ctx = {}) => {
  ...
  if (!c.events.includes("needs")) return false;

  const prev = String(ctx.prevState || "");
  const next = String(ctx.nextState || "");
  if (next !== STATES.NEEDS) return false;
  if (prev === STATES.NEEDS) return false;
