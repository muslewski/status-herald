...
export const buildArgs = {
  ...
  // Session-scoped hook (e.g. window-unlinked). tmux 3.x has no after-kill-window.
  setHook: (sess, name, command) => [
    "set-hook",
    "-t",
    sess,
    name,
...
/** Windows in one session: [{ id, name }]. Empty if session is gone. */
export const listWindowsInSession = (sess) => {
  const raw = run(buildArgs.listWindowsInSession(sess));
  if (!raw) return [];
  return raw
    .split("\n")
    .map((l) => {
...
export const setHook = (sess, name, command) => {
  run(buildArgs.setHook(sess, name, command));
};
export const setDetachOnDestroy = (sess) => {
  run(buildArgs.setDetachOnDestroy(sess));
};
