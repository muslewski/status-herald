import { denizenCel, tierFor } from "../curtain/denizens.mjs";
...
export const renderCard = (
  ...
  if (denizensOn && theatrics.entity) {
    ...
    if (tier !== "none") {
      ...
      if (cel.length) {
        // Trim common leading indent so the silhouette centers on true ink width.
        const ink = cel.filter((l) => l.trim() !== "");
        const indent = ink.length
          ? Math.min(...ink.map((l) => l.length - l.trimStart().length))
          : 0;
        const trimmed =
          indent > 0 ? cel.map((l) => (l.length >= indent ? l.slice(indent) : l)) : cel;
        const celW = Math.max(0, ...trimmed.map((l) => l.length));
        const celH = trimmed.length;
        // Horizontal: true center on card. Vertical: center within the upper
        // band (above the status block) so the creature sits in the stage, not
        // jammed under the top margin or into the label.
        const bandH = Math.max(celH, Math.floor(rows * 0.42));
        const top = Math.max(0, Math.floor((bandH - celH) / 2));
        const left = Math.max(0, Math.floor((cols - celW) / 2));
        denizenZone = { top, left };
        denizen = trimmed;
      }
    }
  }

  // Build PLAIN geometry first when theatrics will composite (spark/draw/denizen),
  ...
  // Composite stage curtain / spark rain over plain geometry, then color.
