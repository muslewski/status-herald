...
export const arm = (sess, t = realTmux) => {
  ...
  // A stale count would hold the next idle_prompt at WORKING forever.
...
export const stampFromHook = (pane, ev, nowSec, t = realTmux) => {
  ...
  // Notification payloads carry no background_tasks, so the folding rules read
  // what the last task-bearing event left behind.
  const stored = {
    subagents: Number(t.getSessOpt(sess, "@herald_bg_subagents")) || 0,
    shells: Number(t.getSessOpt(sess, "@herald_bg_shells")) || 0,
  };
  t.setSessOpt(sess, "@herald_state", nextState(cur, ev, stored));
