...
- **Bar wash:** transparent `status-style bg=default` + sliding horizontal comet on `status-left` via `#{@herald_bar_line}` (not full-bar colour flash). Disable with `curtain.tmuxBar.wash: false`.
