...
import { gridUp, gridDown } from "../lib/curtain/grid.mjs";
...
test("gridUp/gridDown lifecycle on an isolated tmux server", { skip: !hasTmux() }, () => {
  ...
  try {
    ...
    const live = tt(["list-panes", "-t", "grid:grid", "-F", "#{pane_id}"]).split("\n").filter(Boolean);
    const cur = tt(["list-panes", "-t", "grid:_holding", "-F", "#{pane_id}"]).split("\n").filter(Boolean);
    assert.equal(live.length, 2, "grid window should have exactly 2 panes");
    assert.equal(cur.length, 2, "_holding window should have exactly 2 panes");
    ...
    assert.equal(getOpt(live[0], "@herald_peer"), cur[0], "live0's peer should be cur0");
    assert.equal(getOpt(cur[0], "@herald_peer"), live[0], "cur0's peer should be live0");
    ...
    const liveAfter = tt(["list-panes", "-t", "grid:grid", "-F", "#{pane_id}"]).split("\n").filter(Boolean);
    const curAfter = tt(["list-panes", "-t", "grid:_holding", "-F", "#{pane_id}"]).split("\n").filter(Boolean);
    assert.equal(liveAfter.length, 2, "grid window should still have exactly 2 panes after idempotent gridUp");
    assert.equal(curAfter.length, 2, "_holding window should still have exactly 2 panes after idempotent gridUp");
