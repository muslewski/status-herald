...
// Absolute node + package bin — same rationale as install.mjs hookCommand():
// tmux run-shell often has no nvm/npm-global on PATH, so bare `herald` exits 127.
const HERALD_BIN = fileURLToPath(new URL("../../bin/herald", import.meta.url));

// Pure builder for pane-focus-in/out run-shell bodies. Exported for unit tests.
...
export const gridUp = ({ slots = 2, cmd } = {}) => {
  ...
  // Single-quote the run-shell body so tmux keeps quoted node/bin paths intact.
  t([
    "set-hook",
    "-t",
    SESSION,
    "pane-focus-in",
    `run-shell '${focusHookCmd("focus-in")}'`,
  ]);
  t([
    "set-hook",
    "-t",
    SESSION,
    "pane-focus-out",
    `run-shell '${focusHookCmd("focus-out")}'`,
  ]);
