...
import { isWatchEnd, isWatchStart, nextState, resetsElapsed } from "./hook.mjs";
...
/** Next watcher count after one event (Grok /loop, monitor, bg shells). */
export const nextWatchers = (stored, ev) => {
  let n = Math.max(0, Number(stored) || 0);
  if (isWatchStart(ev)) n += 1;
  if (isWatchEnd(ev)) n = Math.max(0, n - 1);
  return n;
};
...
export const stampFromHook = (
  ...
  // Grok /loop + monitor outlive Stop — track as watchers (not Claude shells).
  const watchers = nextWatchers(t.getSessOpt(sess, "@herald_bg_watchers"), ev);
