...
export const stageDrawEnabled = (sess, cfg = loadConfig().curtain, t = realTmux) => {
  if (motionDisabled(cfg?.animation || {})) return false;
  const themeName =
    (t && t.getSessOpt?.(sess, "@herald_theme")) || themeNameFor(sess, cfg);
...
export const cover = (sess, t = realTmux, cfg = loadConfig().curtain) => {
  ...
  if (stageDrawEnabled(sess, cfg, t)) t.setSessOpt(sess, "@herald_draw", "shut");
...
const monitorIdsFromEv = (ev) => {
  if (Array.isArray(ev.monitorIds) && ev.monitorIds.length) return ev.monitorIds;
...
export const stampFromHook = (
  ...
  if (ev.event === "SessionEnd") {
    ...
    const monKeep = [
      ...new Set([
        ...shAll.filter((id) => knownMon.has(id)),
        ...monFromEv,
      ]),
    ];
