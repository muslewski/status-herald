...
// tmux color markup wrapper for status segments (tmux interprets this in
// status bar). Example: tmuxColor("hi", "colour46") === "#[fg=colour46]hi#[default]"
export const tmuxColor = (text, tmuxColorName) =>
  `#[fg=${tmuxColorName}]${text}#[default]`;

// Visible width, ignoring SGR escapes and tmux markup. Codepoint count (emoji
// may render wider; card centering tolerates a 1-col drift — cosmetic only).
...
export const visibleWidth = (s) =>
  [...s.replace(ANSI_RE, "").replace(/#\[[^\]]*\]/g, "")].length;
