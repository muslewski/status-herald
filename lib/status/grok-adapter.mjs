...
/**
 * Map herald @herald_state (+ armed) to bar status string.
 * Never fabricates "busy" — unarmed/unknown → "unknown".
 * @param {string} heraldState
 * @param {boolean} armed
 * @returns {"working"|"idle"|"needs"|"unknown"}
 */
export function heraldStateToBarStatus(heraldState, armed) {
  if (!armed) return "unknown";
  const s = String(heraldState || "").toLowerCase();
  if (s === "working" || s === "compacting") return "working";
  if (s === "done" || s === "idle") return "idle";
  if (s === "needs") return "needs";
  return "unknown";
}

/**
 * Resolve bar status for a Grok active-session entry from curtain leases.
 * Injectable: opts.heraldStatus(entry) or opts.getSessOpt + sessionNameFor.
 */
export function resolveGrokBarStatus(entry, opts = {}) {
  try {
    if (typeof opts.heraldStatus === "function") {
      return opts.heraldStatus(entry) || "unknown";
    }
    if (typeof opts.getSessOpt === "function") {
      const name =
        (typeof opts.sessionNameFor === "function"
          ? opts.sessionNameFor(entry)
          : "") ||
        entry?.name ||
        "";
      if (!name) return "unknown";
      const armed = opts.getSessOpt(name, "@herald_armed") === "1";
      if (!armed) return "unknown";
      return heraldStateToBarStatus(
        opts.getSessOpt(name, "@herald_state"),
        true,
      );
    }
  } catch {
  ...
  // No tmux/herald wiring: never hardcode busy.
  return "unknown";
}
 ...
 * opts: { grokHome?, activePath?, alive?, getSessOpt?, sessionNameFor?, heraldStatus? }
 */
export function discoverLiveGrokSessions(opts = {}) {
  ...
  for (const a of active) {
    try {
      ...
      const name = path.basename(cwd.replace(/\/+$/, "")) || "grok";
      const entry = { pid, sessionId, cwd, name };
      out.push({
        pid,
        ...
        cwd,
        name,
        status: resolveGrokBarStatus(entry, opts),
        isGrok: true,
        context,
