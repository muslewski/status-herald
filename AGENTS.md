...
- **Grok `/loop` + `monitor` + bg shells:** Stop only ends the main turn. Herald tracks `@herald_bg_watchers` (from `/loop` prompts, `scheduler_create`, `monitor`, bg `run_terminal_command`). Stop/idle with watchers > 0 stays **WORKING** (card shows `m:ss · N loop`). Clear via `scheduler_delete` / kill tools. Quiet settle never fires while watching.
