...
export const focus = (title, t = realTmux) => {
  if (
    ...
    for (const s of t.snapshotArmed()) {
      ...
      if (title && liveName === title) revealFrom(s, t);
      else coverFrom(s, t, names);
    }
    return;
  }
...
export const stampFromHook = (pane, ev, nowSec, t = realTmux) => {
  ...
  const next = nextState(cur, evc, { subagents: subs, shells });
  t.setSessOpt(sess, "@herald_state", next);
  // Freeze how long the turn ran the moment it settles into DONE, so the card
  // can report "worked m:ss" without the clock ticking on past the finish. Only
  // on the transition in -- recomputing on a later idle_prompt would inflate it.
  ...
  // Heartbeat: when the last hook landed. A frozen non-idle card with an old
  // heartbeat is diagnosable (herald curtain inspect) instead of a silent lie.
