...
// Absolute hook body for session after-kill-window (installed on arm).
// prefix+& kills a window; if that was the last live window, only _curtain
// remains and the grey card would stick forever without this prune.
...
export const arm = (sess, t = realTmux, cfg = loadConfig().curtain) => {
  if (t.getSessOpt(sess, "@herald_armed") === "1") {
    ...
    t.setHook?.(sess, "after-kill-window", afterKillWindowHookCmd());
    return;
  }
  ...
  t.setDetachOnDestroy?.(sess);
  // prefix+& / kill-window: if live work dies and only _curtain remains, prune it.
  t.setHook?.(sess, "after-kill-window", afterKillWindowHookCmd());
};

/**
 * Called from tmux after-kill-window on an armed session.
