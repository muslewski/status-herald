...
import { readAccountUsage } from "./bridge-token-oracle.mjs";
...
export function modelWindow(modelId) {
  ...
  // Grok Build context is 500k (not Claude's 1M). Match family before "1m"
  // heuristics so a grok model string never inherits Opus/Sonnet window size.
  if (mid.includes("grok")) return 500_000;
  const oneM = [
    "opus-4-8",
    "opus-4-7",
    "opus-4-6",
...
export async function buildPerSessionData(
  ...
  let context = computeContext(lines);
  const meta = await readSessionMeta(sessionId, metaDir);
  const modelId = meta.model || modelFromTranscript(lines);
  let modelBadge = shortModelBadge(modelId, meta.effort || "");
  let isGrok = /grok/i.test(modelId || "") || /grok/i.test(modelBadge || "");
  if (panePidForGrok != null) {
    const g = detectGrok(panePidForGrok);
    if (g.isGrok) {
      isGrok = true;
      if (!modelBadge) modelBadge = g.label || "Grok";
    }
  }
  // Grok context window is 500k. Transcript model strings sometimes look like
  // Claude (or empty) when the pane is Grok — force the right denominator so
  // the bar never shows "1M" for a 500k Grok session.
  if (isGrok && context.win !== 500_000) {
    const used = context.used || 0;
    const win = 500_000;
    context = {
      ...context,
      win,
      pct: win ? Math.floor((used * 100) / win) : 0,
    };
  }
...
export async function getAccountGauges(opts = {}) {
  try {
    return await readAccountUsage(opts);
  } catch {
