...
export const infoLines = (
  state,
  { elapsed, subagents, shells, watchers, worked, modelLine, sageZone },
) => {
...
export const renderCard = (
  ...
  if (effects?.breathe) {
    ...
    const t =
      theatrics?.breatheT != null
        ? Number(theatrics.breatheT)
        : tick * 0.5; // ~2 fps → seconds-ish
  ...
  // Build PLAIN geometry first when theatrics will composite (spark/draw), so
  // we can merge without splicing SGR. Recolor after. Classic/static keeps the
