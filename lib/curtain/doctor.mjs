...
import { soundDoctorLine } from "./sound.mjs";
...
export const runDoctor = (deps = {}) => {
  ...
  // Curtain sound (soft — default silent is healthy)
  try {
    const sound =
      deps.sound !== undefined
        ? deps.sound
        : loadConfig().curtain?.sound;
    const line = soundDoctorLine(sound);
    push("curtain-sound", line.ok, false, line.detail, line.fixHint);
  } catch (e) {
    push(
      "curtain-sound",
      true,
      false,
      e?.message || "sound config unreadable (treated as off)",
      "",
    );
  }
